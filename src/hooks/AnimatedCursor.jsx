...
function CursorCore({
  ...
  // FIX: removed `isActive` from dependency array — it was causing an infinite loop
  // because isActive was modified inside the effect and also listed as a dep.
  useEffect(() => {
    ...
    clickableEls.forEach((el) => {
      ...
      const onOver = () => setIsActive(true);
      const onClick = () => { setIsActive(true); setIsActiveClickable(false); };
      const onDown = () => setIsActiveClickable(true);
      const onUp = () => setIsActive(true);
      const onOut = () => { setIsActive(false); setIsActiveClickable(false); };
      ...
      handlers.push({ el, onOver, onClick, onDown, onUp, onOut });
    });

    return () => {
      handlers.forEach(({ el, onOver, onClick, onDown, onUp, onOut }) => {
  ...
  }, [clickables]); // removed isActive — was causing infinite re-render
